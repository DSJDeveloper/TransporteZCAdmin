...
- The deploy script (`scripts/deploy.sh`) generates a unique `BUILD_HASH` (8-char hex) per build.
- `VITE_BUILD_HASH=$BUILD_HASH npm run build` injects it via `vite.config.ts` `define`.
- A `version.json` with `{version, builtAt}` is written to `dist/` after each build and rsynced to the server.
- `App.vue` polls `version.json?t=${Date.now()}` every 5 min and shows a refresh banner if the hash differs from `__APP_BUILD_HASH__`.
- `index.html` has `<meta http-equiv>` Cache-Control/Pragma/Expires tags.
- nginx `location = /index.html` (sibling, not nested) sends `Cache-Control: no-store`.
