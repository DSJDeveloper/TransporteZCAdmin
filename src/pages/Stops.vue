...
import type { Stop, StopForm } from "../services/stopService"
...
const editing = ref<Stop | null>(null)
const saving = ref(false)
const form = ref<StopForm>({ name: "", description: "", status: 0 })
...
const deleting = ref<Stop | null>(null)
const deletingConfirm = ref(false)

const filtered = computed(() => {
  const q = search.value.toLowerCase().trim()
  if (!q) return stopStore.list
  return stopStore.list.filter(
    (s) =>
...
function compare(a: Stop, b: Stop, field: string): number {
...
const columns = [
  ...
  { key: "description", label: "REFERENCIA" },
...
function openCreate() {
  editing.value = null
  form.value = { name: "", description: "", status: 0 }
  clearErrors()
...
function openEdit(s: Stop) {
  editing.value = s
  form.value = { name: s.name, description: s.description, status: s.status }
  clearErrors()
  dialogOpen.value = true
}
...
async function save() {
  ...
  try {
    const ok = editing.value
      ? await stopStore.update(editing.value.id, form.value)
      : await stopStore.create(form.value)
    if (ok) {
...
async function doDelete() {
  ...
  const ok = await stopStore.remove(deleting.value.id)
  if (ok) {
  ...
  <div class="p-margin-mobile md:p-margin-desktop min-h-screen">
    ...
    <div class="mb-lg">
      <div class="flex items-center gap-xs text-label-md text-outline mb-xs">
        ...
        <span class="text-primary font-bold">Paradas</span>
      </div>
      <h2 class="text-headline-lg text-on-surface font-bold">Gestión de Paradas</h2>
      <p class="text-body-lg text-on-surface-variant">
        Administre las paradas disponibles para las rutas
      </p>
    </div>
    ...
    <section class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm overflow-hidden">
      <div class="p-md md:p-lg border-b border-outline-variant flex flex-col md:flex-row md:items-center md:justify-between gap-md bg-surface-container-low/30">
        <h4 class="text-headline-sm font-semibold text-on-surface">Lista de Paradas</h4>
        <button
          v-if="isAdmin"
      ...
      <div v-else-if="!filtered.length" class="p-xl text-center text-on-surface-variant">
        ...
        <p class="mt-2">No se encontraron paradas</p>
      </div>
      ...
      <template v-else>
        ...
        <div class="hidden md:block overflow-x-auto">
          <table class="w-full text-left font-body-md text-body-md border-collapse">
            ...
            <tbody class="divide-y divide-outline-variant">
              <tr v-for="s in paginated" :key="s.id" class="hover:bg-primary-container/5 transition-colors group">
                <td class="px-lg py-md text-outline font-bold">{{ String(s.id).padStart(2, "0") }}</td>
                <td class="px-lg py-md text-on-surface font-bold">{{ s.name }}</td>
                <td class="px-lg py-md text-on-surface-variant">{{ s.description || "—" }}</td>
                ...
                <td v-if="isAdmin" class="px-lg py-md text-right">
                  <div class="flex justify-end gap-xs opacity-40 group-hover:opacity-100 transition-all duration-200">
                    <button class="p-xs hover:bg-primary/10 rounded-lg text-primary transition-colors" title="Editar" @click="openEdit(s)">
                      <span class="material-symbols-outlined">edit</span>
                    </button>
                    <button class="p-xs hover:bg-error/10 rounded-lg text-error transition-colors" title="Eliminar" @click="confirmDelete(s)">
        ...
        <div class="md:hidden divide-y divide-outline-variant">
          <div v-for="s in paginated" :key="s.id" class="p-md space-y-sm">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-sm">
                ...
                <div>
                  ...
                  <span v-if="s.description" class="ml-2 text-on-surface-variant text-[12px]">{{ s.description }}</span>
                </div>
              </div>
              <span
                v-if="s.status === 0"
      ...
      <div class="relative bg-surface-container-lowest rounded-xl shadow-2xl border border-outline-variant w-full max-w-3xl mx-auto p-md md:p-xl max-h-[92vh] overflow-y-auto">
        ...
        <form @submit.prevent="save" class="space-y-lg">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-lg">
            ...
            <div class="space-y-base">
              <label class="font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Referencia</label>
              <input
    ...
    <div v-if="deletingConfirm" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-surface-container-lowest rounded-xl shadow-2xl border border-outline-variant w-full max-w-sm mx-auto p-md md:p-xl">
        <div class="flex items-start gap-md mb-lg">
          ...
          <div>
            <h3 class="font-headline-sm text-headline-sm text-on-surface">Eliminar Parada</h3>
