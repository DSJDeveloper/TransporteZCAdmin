...
import type { Horario, HorarioForm } from "../services/horarioService"
...
const editing = ref<Horario | null>(null)
...
const filtered = computed(() => {
  ...
  if (!q) return horarioStore.list
  return horarioStore.list.filter(
    (h) =>
      h.code.toLowerCase().includes(q) ||
      h.shudle.toLowerCase().includes(q),
  )
})

const totalPages = computed(() => Math.max(1, Math.ceil(filtered.value.length / perPage.value)))

function compare(a: Horario, b: Horario, field: string): number {
...
function validate(): boolean {
  ...
  if (!form.value.shudle.trim()) e.shudle = "El horario es obligatorio"
...
function openCreate() {
  editing.value = null
  form.value = { code: "", shudle: "", status: 0 }
  clearErrors()
...
function openEdit(h: Horario) {
  editing.value = h
  form.value = { code: h.code, shudle: h.shudle, status: h.status }
  clearErrors()
  dialogOpen.value = true
}
...
async function save() {
  ...
  try {
    const ok = editing.value
      ? await horarioStore.update(editing.value.id, form.value)
      : await horarioStore.create(form.value)
    if (ok) {
...
function confirmDelete(h: Horario) {
  deleting.value = h
...
async function doDelete() {
  ...
  const ok = await horarioStore.remove(deleting.value.id)
  if (ok) {
...
onMounted(() => horarioStore.fetchAll())
  ...
  <div class="p-margin-mobile md:p-margin-desktop min-h-screen">
    ...
    <div class="mb-lg">
      <div class="flex items-center gap-xs text-label-md text-outline mb-xs">
        ...
        <span class="text-primary font-bold">Horarios</span>
      </div>
      <h2 class="text-headline-lg text-on-surface font-bold">Gestión de Horarios</h2>
      <p class="text-body-lg text-on-surface-variant">
        Administre los horarios disponibles para las rutas
      </p>
    </div>
    ...
    <section class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm overflow-hidden">
      <div class="p-md md:p-lg border-b border-outline-variant flex flex-col md:flex-row md:items-center md:justify-between gap-md bg-surface-container-low/30">
        <h4 class="text-headline-sm font-semibold text-on-surface">Lista de Horarios</h4>
        <button
          v-if="isAdmin"
      ...
      <div v-else-if="!filtered.length" class="p-xl text-center text-on-surface-variant">
        ...
        <p class="mt-2">No se encontraron horarios</p>
      </div>
      ...
      <template v-else>
          ...
          <table class="w-full text-left font-body-md text-body-md border-collapse">
            <thead class="bg-surface-container-high/20 border-b border-outline-variant">
              <tr>
                ...
                <th v-if="isAdmin" class="px-lg py-md font-bold text-on-surface-variant uppercase text-[11px] tracking-widest text-right">ACCIONES</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-outline-variant">
              <tr v-for="h in paginated" :key="h.id" class="hover:bg-primary-container/5 transition-colors group">
                <td class="px-lg py-md text-outline font-bold">{{ String(h.id).padStart(2, "0") }}</td>
                <td class="px-lg py-md">
                  <code class="bg-surface-container-high px-xs rounded text-primary font-bold">{{ h.code }}</code>
                </td>
                <td class="px-lg py-md text-on-surface font-bold">{{ h.shudle }}</td>
                ...
                <td v-if="isAdmin" class="px-lg py-md text-right">
                  <div class="flex justify-end gap-xs opacity-0 group-hover:opacity-100 transition-opacity">
                    <button class="p-xs hover:bg-primary/10 rounded-lg text-primary transition-colors" title="Editar" @click="openEdit(h)">
                      <span class="material-symbols-outlined">edit</span>
                    </button>
                    <button class="p-xs hover:bg-error/10 rounded-lg text-error transition-colors" title="Eliminar" @click="confirmDelete(h)">
        ...
        <div class="md:hidden divide-y divide-outline-variant">
          <div v-for="h in paginated" :key="h.id" class="p-md space-y-sm">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-sm">
                ...
                <div>
                  <span class="font-bold text-on-surface">{{ h.shudle }}</span>
            ...
            <div v-if="isAdmin" class="flex justify-end gap-xs pt-xs">
              <button
      ...
      <div class="relative bg-surface-container-lowest rounded-xl shadow-2xl border border-outline-variant w-full max-w-lg mx-auto p-md md:p-xl max-h-[90vh] overflow-y-auto">
        ...
        <form @submit.prevent="save" class="space-y-lg">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-lg">
            ...
            <div class="space-y-base">
              <label class="font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Horario</label>
              <input
                v-model="form.shudle"
                :class="[
                  'w-full h-11 px-md bg-surface-container-lowest border rounded-xl transition-all font-body-md text-body-md outline-none',
                ...
                @input="errors.shudle && delete errors.shudle"
              />
              <p v-if="errors.shudle" class="text-error text-[12px] font-bold">{{ errors.shudle }}</p>
            </div>
            <div class="space-y-base">
    ...
    <div v-if="deletingConfirm" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-surface-container-lowest rounded-xl shadow-2xl border border-outline-variant w-full max-w-sm mx-auto p-md md:p-xl">
        <div class="flex items-start gap-md mb-lg">
          ...
          <div>
            <h3 class="font-headline-sm text-headline-sm text-on-surface">Eliminar Horario</h3>
            <p class="text-body-md text-on-surface-variant mt-1">¿Estás seguro de eliminar <strong>{{ deleting?.shudle }}</strong>?</p>
