-- ============================================================
-- HOTFIX: manage_profile — INSERT en profiles en vez de UPDATE
-- ============================================================
-- Bug: La función hacía "UPDATE public.profiles SET name = p_name"
-- asumiendo que el trigger on_auth_user_created ya había creado
-- la fila. Como el trigger estaba declarado ON users (tabla
-- inexistente), nunca se disparaba y el perfil quedaba huérfano.
-- 
-- Fix: INSERT ... ON CONFLICT (id) DO UPDATE SET ...
-- así no depende del trigger. Si el trigger ya creó el perfil,
-- el ON CONFLICT lo actualiza.
--
