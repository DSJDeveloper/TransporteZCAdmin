...
CREATE OR REPLACE FUNCTION public.manage_stop(p_action character varying, p_id bigint DEFAULT NULL, p_name character varying DEFAULT NULL, p_description character varying DEFAULT NULL, p_status integer DEFAULT NULL)
RETURNS json
    LANGUAGE plpgsql
    SECURITY DEFINER
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_stops_by_route(p_route_id bigint)
RETURNS json
    LANGUAGE plpgsql
...
BEGIN
    ...
    RETURN json_build_object(
        'success', true,
        'data', COALESCE((
            SELECT json_agg(row_to_json(rs_data.*))
            FROM (
                SELECT
                    rs.id,
                    rs.route_id,
...
CREATE OR REPLACE FUNCTION public.manage_route_stop(
    ...
    p_idroute_stop bigint DEFAULT NULL,
...
BEGIN
    IF LOWER(p_action) = 'list_by_route' THEN
        IF p_idroute IS NULL THEN
            RETURN json_build_object('success', false, 'message', 'Se requiere p_idroute para listar paradas.');
        END IF;
        SELECT json_agg(row_to_json(rs.*)) INTO v_data FROM (
            SELECT * FROM public.route_stops WHERE route_id = p_idroute ORDER BY stop_order
        ) rs;
        RETURN json_build_object('success', true, 'data', COALESCE(v_data, '[]'::json));
    END IF;

    IF NOT public.is_admin() THEN
        RETURN json_build_object('success', false, 'message', 'Solo administradores pueden gestionar paradas de rutas.');
    END IF;

    IF LOWER(p_action) = 'create' THEN
        SELECT COALESCE(MAX(stop_order), 0) + 1 INTO v_new_order FROM public.route_stops WHERE route_id = p_idroute;
        INSERT INTO public.route_stops (route_id, stop_id, price, stop_order)
        VALUES (p_idroute, p_idstop, p_price, COALESCE(p_stop_order, v_new_order))
        ON CONFLICT DO NOTHING
        RETURNING row_to_json(public.route_stops.*) INTO v_data;
        IF v_data IS NULL THEN
            RETURN json_build_object('success', false, 'message', 'Esta parada ya existe en la ruta.');
        END IF;
        RETURN json_build_object('success', true, 'data', v_data, 'message', 'Parada asignada a la ruta.');

    ELSIF LOWER(p_action) = 'update' THEN
        UPDATE public.route_stops SET
            price = COALESCE(p_price, price),
            stop_order = COALESCE(p_stop_order, stop_order)
        WHERE id = p_idroute_stop
        RETURNING row_to_json(public.route_stops.*) INTO v_data;
        IF v_data IS NULL THEN
            RETURN json_build_object('success', false, 'message', 'Asignacion no encontrada.');
        END IF;
        RETURN json_build_object('success', true, 'data', v_data, 'message', 'Asignacion actualizada.');

    ELSIF LOWER(p_action) = 'delete' THEN
        DELETE FROM public.route_stops WHERE id = p_idroute_stop;
        RETURN json_build_object('success', true, 'message', 'Parada removida de la ruta.');

    ELSIF LOWER(p_action) = 'delete_by_route' THEN
        DELETE FROM public.route_stops WHERE route_id = p_idroute;
        GET DIAGNOSTICS v_affected_count = ROW_COUNT;
        RETURN json_build_object('success', true, 'message', v_affected_count || ' paradas removidas de la ruta.');
    ELSE
        RETURN json_build_object('success', false, 'message', 'Accion no reconocida.');
    END IF;
END;
...
-- 7. manage_client
DROP FUNCTION IF EXISTS public.manage_client(character varying, bigint, character varying, character varying, character varying, character varying, character varying, character varying, character varying, bigint, character varying);
CREATE OR REPLACE FUNCTION public.manage_client(p_action character varying, p_id bigint DEFAULT NULL, p_name character varying DEFAULT NULL, p_documenttype character varying DEFAULT NULL, p_documentid character varying DEFAULT NULL, p_address character varying DEFAULT NULL, p_phone character varying DEFAULT NULL, p_email character varying DEFAULT NULL, p_photo_url character varying DEFAULT NULL, p_idroute bigint DEFAULT NULL, p_auth_user_name character varying DEFAULT NULL)
RETURNS json
    LANGUAGE plpgsql
    SECURITY DEFINER
AS $function$
...
BEGIN
    IF LOWER(p_action) = 'list' THEN
        SELECT json_agg(row_to_json(c.*)) INTO v_client FROM (SELECT * FROM public.clients ORDER BY id) c;
        RETURN json_build_object('success', true, 'data', COALESCE(v_client, '[]'::json));
    END IF;

    IF NOT public.is_admin() AND NOT public.is_admin_or_supervisor() THEN
        RETURN json_build_object('success', false, 'message', 'No tienes permisos para gestionar clientes.');
    END IF;

    IF LOWER(p_action) = 'create' THEN
        IF p_documentid IS NOT NULL AND p_documentid <> '' THEN
            v_normalized_doc := public.normalize_document_id(p_documentid);
            IF EXISTS (SELECT 1 FROM public.clients WHERE documentid = v_normalized_doc) THEN
                RETURN json_build_object('success', false, 'message', 'El documento de identidad "' || p_documentid || '" ya está registrado en el sistema.');
            END IF;
            IF EXISTS (SELECT 1 FROM auth.users au WHERE au.raw_user_meta_data ->> 'documentID' = v_normalized_doc) THEN
                RETURN json_build_object('success', false, 'message', 'El documento de identidad "' || p_documentid || '" ya está registrado en un usuario del sistema.');
            END IF;
        END IF;
        v_clean_phone := REGEXP_REPLACE(COALESCE(p_phone, ''), '[^0-9+]', '', 'g');
        v_final_email := NULLIF(TRIM(COALESCE(p_email, '')), '');
        IF v_final_email IS NOT NULL THEN
            IF EXISTS (SELECT 1 FROM auth.users WHERE email = v_final_email) THEN
                RETURN json_build_object('success', false, 'message', 'El correo electronico "' || v_final_email || '" ya esta registrado en el sistema.');
            END IF;
        END IF;

        INSERT INTO public.clients ("name", "documenttype", "documentid", "address", "phone", "email", "photo_url", "idroute", "auth_user_name")
        VALUES (p_name, p_documenttype, COALESCE(v_normalized_doc, p_documentid), p_address, v_clean_phone, v_final_email, p_photo_url, p_idroute, NULLIF(p_auth_user_name, ''))
        RETURNING id INTO v_new_id;

        SELECT row_to_json(c.*) INTO v_client FROM public.clients c WHERE c.id = v_new_id;
        RETURN json_build_object('success', true, 'data', v_client, 'message', 'Cliente creado exitosamente.');
    ...
    ELSIF LOWER(p_action) = 'update' THEN
        ...
        IF p_documentid IS NOT NULL AND p_documentid <> '' THEN
            v_normalized_doc := public.normalize_document_id(p_documentid);
            IF EXISTS (SELECT 1 FROM public.clients WHERE documentid = v_normalized_doc AND id != p_id) THEN
                RETURN json_build_object('success', false, 'message', 'El documento de identidad "' || p_documentid || '" ya está registrado para otro cliente.');
            END IF;
            IF EXISTS (SELECT 1 FROM auth.users au WHERE au.raw_user_meta_data ->> 'documentID' = v_normalized_doc AND au.id != (SELECT auth_user_id FROM public.clients WHERE id = p_id)) THEN
                RETURN json_build_object('success', false, 'message', 'El documento de identidad "' || p_documentid || '" ya está registrado en otro usuario del sistema.');
            END IF;
        END IF;
        IF v_final_email IS NOT NULL THEN
            IF EXISTS (SELECT 1 FROM auth.users WHERE email = v_final_email AND id != (SELECT auth_user_id FROM public.clients WHERE id = p_id)) THEN
                RETURN json_build_object('success', false, 'message', 'El correo electronico "' || v_final_email || '" ya esta registrado para otro usuario.');
            END IF;
        END IF;
        ...
        UPDATE public.clients SET
            ...
            "documentid" = COALESCE(v_normalized_doc, p_documentid, "documentid"),
            "address" = COALESCE(p_address, "address"),
            "phone" = v_clean_phone,
            "email" = v_final_email,
            "photo_url" = COALESCE(p_photo_url, "photo_url"),
            "idroute" = p_idroute,
            "auth_user_name" = COALESCE(NULLIF(p_auth_user_name, ''), "auth_user_name")
        WHERE id = p_id
        RETURNING row_to_json(public.clients.*) INTO v_client;

        IF v_client IS NULL THEN
            RETURN json_build_object('success', false, 'message', 'Cliente no encontrado.');
        END IF;

        RETURN json_build_object('success', true, 'data', v_client, 'message', 'Cliente actualizado exitosamente.');
...
BEGIN
    ...
    -- Tarifa base de la empresa (para conversión)
    ...
    -- Si no viene precio directo pero sí idstop, resolver precio de la parada desde BD
    IF p_unit_price IS NULL AND p_idstop IS NOT NULL THEN
        SELECT rs.price INTO p_unit_price
        FROM public.route_stops rs
        WHERE rs.id = p_idstop;
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_transactions) LOOP
        ...
        -- 2. Buscar y bloquear cliente (obteniendo balance y tickets)
        SELECT id, balance, tickets, "creditLimit", name
        INTO v_client_id, v_current_balance, v_current_tickets,
             v_credit_limit_raw, v_client_name
        FROM public.clients
        ...
        -- 3. Resolver precio aplicable
        v_price_context := COALESCE(v_item_price, p_unit_price, v_global_ticket);

        IF v_price_context <= 0 THEN
            RAISE EXCEPTION 'Precio de parada/pasaje invalido (%).', v_price_context;
        END IF;

        -- 4. Cálculo dual: Monto en $ y Tickets equivalentes
        ...
        -- 6. Actualizar cliente (monto y tickets)
        UPDATE public.clients
        SET balance = v_new_balance,
            tickets = v_new_tickets
        WHERE id = v_client_id;

        -- 7. Generar UID de transacción
        v_tx_uid := TO_CHAR(NOW(), 'YYMMDDHH24MISS')
                    || FLOOR(RANDOM() * 100)::TEXT
                    || v_processed_count::TEXT;

        -- 8. Insertar en transactions con esquema dual completo
        INSERT INTO public.transactions (
            uid,
            idclient,
            "createBy",
    ...
    RETURN json_build_object(
        ...
        'message',           'Lote de transacciones procesado con exito.',
        'processed_records', v_processed_count,
        'unit_fare',         COALESCE(p_unit_price, v_global_ticket),
        'details',           v_response_data
    );
