-- ============================================================
-- DIAGNÓSTICO: Capturar qué recibe manage_profile
-- 
-- 1. Ejecutar TODO este script en el SQL Editor
-- 2. Ir a la app y actualizar la contraseña del chofer
-- 3. Volver al SQL Editor y hacer:
--       SELECT * FROM debug_manage_profile ORDER BY ts DESC LIMIT 5;
-- ============================================================

DROP TABLE IF EXISTS debug_manage_profile;
CREATE TABLE debug_manage_profile (
  ts TIMESTAMPTZ DEFAULT NOW(),
  action VARCHAR,
  user_id UUID,
  email VARCHAR,
  password_length INT,
  password_first_ascii INT,
  password_last_ascii INT,
...
DROP FUNCTION IF EXISTS public.manage_profile(VARCHAR, UUID, VARCHAR, VARCHAR, VARCHAR, VARCHAR);
